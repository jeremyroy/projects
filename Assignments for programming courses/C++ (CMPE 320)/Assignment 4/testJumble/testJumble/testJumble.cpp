...
void testJumble() {
	cout << "FYI I added lowercase converstion to the name and difficulty fields of the constructors.  To disactivate this functionality, comment out line 125 of jumble.cpp" << endl << endl;
	
	// Test error handling
	//invalid character (symbol)
	try{
		JumblePuzzle error("Hello!", "easy");
	}
	catch (BadJumbleException& e){
		cerr << e.what() << endl;
	}//end bloc
	...
	try{
		JumblePuzzle error("Hel1lo", "easy");
	}
	catch (BadJumbleException& e){
		cerr << e.what() << endl;
	}//end bloc
	...
	//Too many letters
	try{
		JumblePuzzle error("anticonstitutionally", "easy");
	}
	catch (BadJumbleException& e){
		cerr << e.what() << endl;
	}//end bloc
	//Illegal difficulty
	try{
		JumblePuzzle error("hello", "difficult");
	}
	catch (BadJumbleException& e){
		cerr << e.what() << endl << endl;
	}//end bloc

	// Test error handeling in non-necessary add-on functions
	cout << "Testing public mutators:" << endl << "Original jumble:" << endl;
	JumblePuzzle jp0("TESTWORD", "easy");
	showJumble(jp0.getJumble(), jp0.getSize());
	cout << "Changing word... entire jumble should be different:" << endl;
	jp0.setNewWord("NEWWORD");
	showJumble(jp0.getJumble(), jp0.getSize());
	cout << "Changing difficulty... entire jumble should be different:" << endl;
	jp0.setNewDifficulty("hard");
	showJumble(jp0.getJumble(), jp0.getSize());
	// Test error handeling in non-necessary add-on functions
	// Invalid new difficulty:
	try{
		jp0.setNewDifficulty("difficult");
	}
	catch (BadJumbleException& e){
		cerr << e.what() << endl;
	}//end block
	...
	cout << "Testing copy constructor:" << endl <<"jp1" << endl;
	...
	cout << "Should look the same:" << endl << "jp2" << endl;
	...
	cout << "Testing assignment operator:" << endl << "jp3" << endl;
	JumblePuzzle jp3("BYE", "easy");
	showJumble(jp3.getJumble(), jp3.getSize());
	JumblePuzzle jp4 = jp3;
	cout << "Should look the same:" << endl << "jp4" << endl;
